...
// This service creates new class identifiers for fact entities.
// Each fact cluster must have one node pointing to the node that is common to all facts.
// This creates a cluster of facts inside the all encompassing semantic graph.
...
// CreateNewIdentifier created new class identifier to be used as part of a fact cluster.
func (i *IdentifierCreator) CreateNewIdentifier() (uint, error) {
	...
	newIdentifier, err := classEntity.CreateFact()
	if err != nil {
		return 0, err
	}
