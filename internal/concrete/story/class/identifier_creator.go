...
// Each position cluster must have one node pointing to the node that is common to all storys.
// This creates a cluster of storys inside the all encompassing semantic graph.
...
func (i *IdentifierCreator) CreateNewIdentifier() (uint, error) {
	...
	newIdentifier, err := classEntity.CreateStory()
	if err != nil {
		return 0, err
	}
